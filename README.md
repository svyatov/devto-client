...
| Privilege-gated | every `/api/surveys` operation, reads included | Admin-gated. The tier table called the reads api-key, which was wrong at both pins: the controller authorizes admin before every action, and an ordinary key gets 401 on the plain list. |
